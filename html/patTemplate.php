...
*/
	function createParser($fname) {
		...
		//	If basedir is set and no http URI is given, prepend basedir
		...
		while (!feof($fp)) {
			...
			if (preg_match("!<patTemplate:([a-z]+)[\s]*([^>]*)?>!i", $line, $regs)) {
			...
			} elseif (preg_match("!<\/patTemplate:([a-z]+)>!i", $line, $regs)) {
				//	========= [ CLOSING TAG ] =========
...
*/
	function startElementHandler($fname, $tagname, $attributes, $line, $lineno) {
		...
		} elseif( isset( $this->whitespace[(count($this->whitespace)-1)] ) ) {
		...
		switch ($tagname) {
			...
			case "tmpl":
				...
				$tmpl_name	=	strtoupper( $attributes["name"] );
				...
				if ( isset( $attributes["type"] ) ) {
					...
					$this->template_types[$this->depth] =	$attributes["type"];
				} else {
				...
				switch ($this->template_types[$this->depth]) {
					...
					case "CONDITION":
						...
						$this->setConditionVar($this->template_names[$this->depth], $attributes["conditionvar"]);
						break;
					//	Template is a simple condition Tenplate => it needs required vars
					case "SIMPLECONDITION":
						//	none found => there is an error
						if (isset( $attributes["requiredvars"] ) ) {
							$this->setAttribute($this->template_names[$this->depth], "requiredvars", explode(",", $attributes["requiredvars"] ));
						} else {
				...
				if ($this->depth > 0) {
					...
					if (isset($attributes["placeholder"])) {
						...
						//	Tell the parent template, that it has to parse the child template, before parsing itself 
	...
	function getVars($template) {
		...
		if (is_array($this->variables[$template])) {
			...
			foreach( $this->variables[$template] as $variable => $value ) {
				...
				if (is_array($value)) {
					if( isset( $value[$this->iteration[$template]] ) )
...
*
* @param	string	$name name of template - repetitive argument
* @access	public
*/
	function dump() {
		...
		echo	"		<td class=\"head\" valign=\"top\" colspan=\"2\">Globals</td>\n";
		echo	"	</tr>\n";
		...
		if( func_num_args() ) {
			for( $i = 0; $i < func_num_args(); ++$i ) {
				$tmpl	=	func_get_arg( $i );
				array_push( $tmpls, strtoupper( $tmpl ) );
			}
		}
		else {
			for( $i = 0; $i < $this->cnt_templates; ++$i ) {
				array_push( $tmpls, $this->templates[$i] );
			}
		}
		
		foreach( $tmpls as $name ) {
			...
			while( list( $key, $value ) = each( $this->attributes[$name] ) ) {
				...
				// requiredevars are stored an array
				if( $key == "requiredvars" ) {
					$value	=	implode( ", ", $value );
				}
			...
			if ($this->cnt_subtemplates[$name] > 0) {
				...
				if (is_array($unused) && count($unused) > 0) {
					...
					//	Display all Variables in table
					for ($k = 0; $k<count( $unused ); $k++) {
						if( strncmp( $this->tag_start."TMPL:", $unused[$k], strlen( $this->tag_start."TMPL:" ) ) == 0 )
							continue;
						echo	"				<tr>\n";
						echo	"					<td class=\"text\">".$unused[$k]."</td>\n";
						echo	"				</tr>\n";
