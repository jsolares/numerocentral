...
if ( $allowed ) {
	...
	if ( $request == 8 ) {
		$db -> query ("select saldo_minutos,saldo_qtz from users, saldo where users.uid = saldo.uid and accountcode = '$accountcode'");
		$db -> next_record();
		echo $db -> f ("saldo_minutos") . " m|Q " . number_format($db -> f ("saldo_qtz"),2,".",""). "";
	}
} else {
	echo "fail";
}
